...
class MMDataset(Dataset):
    ...
    def __init_mosi(self):
        ...
        if self.args['need_data_aligned']:
            # Aligned audio/vision occupy the word positions between [CLS] and [SEP].
            # Zero rows inside that span are missing observations, not padding.
            bert_mask = data[self.mode]['text_bert'][:, 1, :].astype(bool)
            word_mask = bert_mask.copy()
            word_mask[:, 0] = False
            sep_positions = bert_mask.sum(axis=1) - 1
            word_mask[np.arange(len(word_mask)), sep_positions] = False
            self.text_mask = bert_mask
            self.audio_mask = word_mask & np.any(np.isfinite(self.audio) & (self.audio != 0), axis=2)
            self.vision_mask = word_mask & np.any(np.isfinite(self.vision) & (self.vision != 0), axis=2)
        else:
            self.text_mask = data[self.mode]['text_bert'][:, 1, :].astype(bool)
            if self.args['feature_A'] != "":
    ...
    def __getitem__(self, index):
        sample = {
            ...
            'text_mask': torch.from_numpy(self.text_mask[index].copy()),
            'index': index,
            'id': self.ids[index],
            'labels': {k: torch.Tensor(v[index].reshape(-1)) for k, v in self.labels.items()}
        } 
        if self.args['need_data_aligned']:
            sample['audio_mask'] = torch.from_numpy(self.audio_mask[index].copy())
            sample['vision_mask'] = torch.from_numpy(self.vision_mask[index].copy())
        else:
            sample['audio_lengths'] = self.audio_lengths[index]
            sample['vision_lengths'] = self.vision_lengths[index]
        return sample

def MMDataLoader(args, num_workers, include_test=False):
    ...
    if include_test:
        datasets['test'] = MMDataset(args, mode='test')
